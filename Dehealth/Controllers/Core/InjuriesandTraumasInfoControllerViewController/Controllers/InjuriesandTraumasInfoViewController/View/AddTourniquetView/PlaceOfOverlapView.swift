...
protocol PlaceOfOverlapViewDelegate: AnyObject {
    func getTitleOfSelect(_ title: Int)
}

class PlaceOfOverlapView: UIView {
    // MARK: - Properties
    weak var delegate: PlaceOfOverlapViewDelegate?
    ...
    private lazy var handsSegmentedControl: UISegmentedControl = {
        let sc = UISegmentedControl(items: HandSegment.allCases.map { $0.title })
    ...
    private lazy var legsSegmentedControl: UISegmentedControl = {
        let sc = UISegmentedControl(items: LegSegment.allCases.map { $0.title })
        sc.backgroundColor = .white
        sc.selectedSegmentTintColor = .clear // Set to clear to apply custom image
        sc.setBackgroundImage(UIImage(color: .black700!), for: .selected, barMetrics: .default)
        sc.setBackgroundImage(UIImage(color: .black200!), for: .normal, barMetrics: .default)
    ...
    enum HandSegment: Int, CaseIterable {
    ...
    enum LegSegment: Int, CaseIterable {
    ...
    private func configureUI() {
        ...
        dividerBottomView.anchor(top: legsSegmentedControl.topAnchor, bottom: legsSegmentedControl.bottomAnchor, width: 1)
        dividerBottomView.centerX(inView: legsSegmentedControl)
    }
    ...
    @objc private func segmentChanged(_ sender: UISegmentedControl) {
        ...
        if let title = sender.titleForSegment(at: sender.selectedSegmentIndex) {
            ...
            } else if title == "Права рука" {
                delegate?.getTitleOfSelect(1)
            } else if title == "Ліва нога" {
                delegate?.getTitleOfSelect(2)
            } else if title ==  "Права нога" {
                delegate?.getTitleOfSelect(3)
            }
        }
    }
}
